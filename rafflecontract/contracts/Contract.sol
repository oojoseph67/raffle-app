...
contract NFTRaffleContract {
    ...
    event NFTPrizeSet(address nftAddress, uint256 nftId, uint256 entryCost);
    ...
    function startRaffle(address _nftContract, uint256 _tokenID, uint256 _entryCost) public onlyOwner {
        require(!raffleStatus, "Raffle has already started");
        require(nftAddress == address(0), "NFT prize already set. Please select winner");
        require(
            ERC721Base(_nftContract).ownerOf(_tokenID) == owner, "This is not your NFT"
        );
        ...
        raffleStatus = true;
        emit RaffleStarted();
        emit NFTPrizeSet(_nftContract, _tokenID, _entryCost);
    }
    ...
    function selectWinner() public onlyOwner {
        ...
        require(nftAddress != address(0), "NFT prixe not set");

        bytes32 blockHash = blockhash(block.number - playerSelector.length);
    ...
    function setNewEntryCost(uint256 _entryCost) public onlyOwner {
        require(!raffleStatus, "Raffle is still running");

        entryCost = _entryCost;
        emit EntryCostChanged(_entryCost);
    }
